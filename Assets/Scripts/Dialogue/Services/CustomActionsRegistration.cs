...
namespace Dialogue.Services
{
	...
	public class CustomActionsRegistration : IActionRegistration
	{
		internal class CommandRegistration : ICommand
		{
			...
			public bool TryParseArgs(string[] args, out object[] result, out string message)
			{
				...
				for (var i = argumentCount; i < finalArgs.Length; i++)
				{
			...
			private (int Min, int Max) ParameterCount
			{
				get
				{
					...
					var min = parameters.Length - optional;
					var max = parameters.Length;
					return (min, max);
				}
			}

			// internal CommandDispatchResult Invoke(List<string> parameters, out Coroutine commandCoroutine)
			// {
			// 	object target;
			//
			...
			// 		var targetComponent = gameObject.GetComponent(DeclaringType);
			//
			...
			// 			return new CommandDispatchResult
			// 			{
			// 				Message = $"{Name} can't be called on {gameObjectName}, because it doesn't have a {DeclaringType.Name}",
			// 				Status = CommandDispatchResult.StatusType.TargetMissingComponent
			// 			};
			// 		}
			//
			...
			public string UsageString
			{
				get
				{
					...
					foreach (var parameter in Method.GetParameters())
					{
						...
						if (_typeFriendlyNames.TryGetValue(type, out var typeName) == false)
						{
							typeName = type.Name;
						}

						var displayName = $"{parameter.Name} <i>({typeName})</i>";
			...
			private readonly Dictionary<Type, string> _typeFriendlyNames = new Dictionary<Type, string>
			{
				{
					typeof(int), "number"
				},
		...
		public CustomActionsRegistration(Library library)
		{
			Library = library;
		}
		...
		private static Func<string, object>[] CreateConverters(MethodInfo method)
		{
			...
			var result = new Func<string, object>[parameterInfos.Length];
		...
		private static Func<string, object> CreateConverter(ParameterInfo parameter, int index)
		{
			...
			if (typeof(Component).IsAssignableFrom(targetType))
			{
				return arg =>
				{
					var gameObject = GameObject.Find(arg);
			...
			if (typeof(bool).IsAssignableFrom(targetType))
			{
				return arg =>
				{
					...
					if (Equals(parameter.Name, StringComparison.InvariantCultureIgnoreCase))
					{
						return true;
					}
		...
		private static readonly HashSet<Action<IActionRegistration>> ActionRegistrationMethods = new HashSet<Action<IActionRegistration>>();
		...
		private class LibraryRegistrationProxy : IActionRegistration
		{
			...
			public void AddFunction(string name, Delegate implementation)
			{
				...
				if (_library.FunctionExists(name))
				{
				...
				_library.RegisterFunction(name, implementation);
			}
