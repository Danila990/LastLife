...
namespace Installer.Sandbox
{
	...
	[CreateAssetMenu(menuName = SoNames.INSTALLERS + nameof(SandboxUiInstaller), fileName = nameof(SandboxUiInstaller))]
	public class SandboxUiInstaller : ScriptableObjectInstaller
	{
		[TitleGroup("Canvas")]
		...
		private void ConfigureUiViews(IContainerBuilder builder, Canvas canvas)
		{
			...
			//Other
			builder.RegisterUiView<BossHealthUiController, BossHealthUiView>(_bossHealthUiView, canvas.transform);
		...
		{
			builder.Register<SandboxMainWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();			
			
			builder.Register<SpawnMenuWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();			
			
			builder.Register<CharacterMenuWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();
			
			builder.Register<QuestsMenuWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();
			
			builder.Register<ShopMenuWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();
						
			builder.Register<SettingsMenuWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();
			
			builder.Register<IntermediateDeathWindow>(Lifetime.Singleton)
				.AsImplementedInterfaces().AsSelf();
		}
