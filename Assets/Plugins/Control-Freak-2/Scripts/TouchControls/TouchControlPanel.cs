...
//! \cond
	...
	static public int 
		...
		public IList<SystemTouch> Touches => touchList;
...
//! \cond 
	...
	// -----------------------
		...
		private SystemTouch StartNewTouch(SystemTouchEventData data)
			...
			if (newTouch != null)
				...
				newTouch.touch.Start(data.pos, data.pos, data.cam, data.isMouseEvent, false, pressure);
	...
	public class SystemTouch 
		...
		public void Update()
			...
			public void From(SystemTouch readOnly)
			{
				touch = readOnly.touch;
				hwId = readOnly.hwId;
				elapsedSinceLastAction = readOnly.elapsedSinceLastAction;
				startFrame = readOnly.startFrame;
			}
		}
